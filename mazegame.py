...
def player_move(i_change, j_change, map_i, position_i, position_j, score, view_dist, map_level):
    ...
    elif map_i[position_i + i_change][position_j + j_change] == "O":
            ...
            while next_map == l:
                ...
                if ch == "s":
                    ...
                    if view_distance <= 3:
                        ...
                        score=score+view_distance
                if ch == "p":
                    print("Would you like to Save the game or Load the previous? S / L : ")
                    ch=readchar.readchar().lower()
                    if ch == "s":
                        save_game(i,j,score,view_distance,cheat,l)
                        print("Game saved.")
                        time.sleep(1)
                    if ch == "l":
                        data=loadfile("savegame.txt")[-6:]
                        print("Loading game...")
                        time.sleep(1)
                        i = int(data[0])
                        j = int(data[1])
                        score = int(data[2])
                        view_distance = int(data[3])
                        if data[4] == "True":
                            cheat = True
                        else:
                            cheat = False
...
def save_game(i_position, j_position, highscore, view_distance, cheater,level):
    with open("savegame.txt", "a") as save:
        save.write(str(i_position) + "\n")
        save.write(str(j_position) + "\n")
        save.write(str(highscore) + "\n")
        save.write(str(view_distance) + "\n")
        save.write(str(cheater) + "\n")
